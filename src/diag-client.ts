...
const mudPort = Number(process.argv[3] ?? '6666');

if (!Number.isFinite(mudPort) || mudPort < 1) {
    console.error('用法: npx tsx src/diag-client.ts <mudHost> <mudPort>');
    process.exit(1);
}

console.log('网关:', GW, '→ MUD:', mudHost, mudPort);
...
ws.on('open', () => {
    ws.send(
        JSON.stringify({
            ...
            connect: { ip: mudHost, port: String(mudPort), wsPath: '/' }
        })
    );
    console.log('已发送握手');
});
